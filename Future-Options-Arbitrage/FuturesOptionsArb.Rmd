...
if(callArbPrice <= 0){
  ...
  sprintf('The best Arbitarage is $%.2f per contract at %dCE strike'
          ,callArbPrice,bestCallStrike)
}
```
...
if(putArbPrice <= 0){
  ...
  sprintf('The Arbitarage is $%.2f per contract at the %dPE strike'
          ,putArbPrice,bestPutStrike)
}
```
...
if(callArbPrice <= 0){
  print('As said above, There is no arbitrage in Call Options')
}else{
  callArbs =callData[-callMax,]

callArbs = callArbs |> 
              filter(ArbAmt > 0) |>
              select(Strike,ArbAmt) |>
              arrange(desc(ArbAmt))

row.names(callArbs) = 1:nrow(callArbs)

knitr::kable(callArbs, col.names = c('Strike Price', 'Arbitrage Price'),
             align = 'c') |>
  kable_styling()
}
```
...
```{r, echo=FALSE}
if(putArbPrice <= 0){
  print('As said above, There is no arbitrage in Put Options')
}else{
  putArbs =putData[-putMax,]
...
row.names(putArbs) = 1:nrow(putArbs)

knitr::kable(putArbs, col.names = c('Strike Price', 'Arbitrage Price'),
             align = 'c') |>
  kable_styling()
}
